...
if [ -d "~/.Ignition/" ]; then
  ...
  rm -r txleveldb
  cp -r * ~/ignitionbackup
  echo "- Backed up files (including wallet.dat) are in ~/ignitionbackup -"
fi
echo "Upgrade Complete - You can now run ignitiond or fill out the config file in ~/Ignition"
